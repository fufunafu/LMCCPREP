-- Keep incomplete or still-flagged bank content out of learner-facing reads.
-- Distribution-rights verification and learner readiness are independent states.

create or replace function question_is_learner_ready(p_qid int)
returns boolean
language sql
stable
security definer
...
as $$
  select coalesce((
    select
      ...
      and q.answer_index >= 0
      and q.answer_index < jsonb_array_length(q.options)
      and (
        not q.has_figure
        or nullif(btrim(q.figure_url), '') is not null
        or exists (
          select 1
          from qbank_question_images qi
          where qi.qid = q.qid
            and qi.distribution_rights_status in ('original', 'licensed', 'verified')
        )
      )
    from questions q
    where q.qid = p_qid
...
revoke all on function question_is_learner_ready(int) from public;
grant execute on function question_is_learner_ready(int) to anon, authenticated;
...
create policy "content readable by entitled users" on questions
  ...
  using (
    ...
    and (
      ...
      or (
        source <> 'user'
        and question_is_learner_ready(qid)
        and (
...
create policy "qbank images readable by entitled users"
  ...
  using (
    (select has_billing_access())
    and distribution_rights_status in ('original', 'licensed', 'verified')
    and exists (
      select 1
      from questions q
      where q.qid = qbank_question_images.qid
        and q.source <> 'user'
        and question_is_learner_ready(q.qid)
        and (
