...
const RIGHTS = ["original", "licensed", "unverified", "quarantined"];
const EDITORIAL = ["pending", "reviewed", "stale", "personal"];

function statusBadge(value: string) {
  const good = value === "original" || value === "licensed" || value === "reviewed";
...
export default async function AdminQuestionsPage({ searchParams }: { searchParams: Promise<{ subject?: string; rights?: string; editorial?: string; q?: string; page?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <form className="flex flex-wrap gap-2 text-sm">
        ...
        <select name="subject" defaultValue={params.subject ?? ""} className="h-9 rounded-lg border bg-background px-3"><option value="">All subjects</option>{result.subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select>
