...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string }> }) {
  ...
  const signupOpen = billingConfigured();
    ...
    <main id="main-content" tabIndex={-1} className="grid min-h-screen bg-[#f7faf9] outline-none dark:bg-[#07110e] lg:grid-cols-[1fr_1.05fr]">
      <div className="flex min-h-screen flex-col px-5 py-6 sm:px-10 lg:px-16">
        ...
        <div className="my-auto w-full max-w-md self-center py-12">
          ...
          <Card className="border-slate-200/80 shadow-xl shadow-slate-900/5 dark:border-white/10"><CardHeader className="pb-6"><CardTitle className="text-2xl tracking-tight">Welcome back</CardTitle><CardDescription>Sign in to continue your MCCQE preparation.</CardDescription></CardHeader><CardContent><Suspense><LoginForm /></Suspense><p className="mt-6 text-center text-sm text-muted-foreground">{signupOpen ? <>New here? <Link href={signupHref} className="font-medium text-emerald-700 hover:underline dark:text-emerald-400">Create an account</Link></> : <>Need an invitation? <Link href="/request-access" className="font-medium text-emerald-700 hover:underline dark:text-emerald-400">Request access</Link></>}</p></CardContent></Card>
        </div>
        <p className="text-center text-xs text-muted-foreground">{signupOpen ? "Subscriptions are billed in CAD through Stripe." : "Private access for invited learners only."}</p>
      </div>
      <div className="relative hidden overflow-hidden bg-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative max-w-lg"><h1 className="text-4xl font-semibold leading-tight tracking-[-0.04em]">A clearer way to turn practice into progress.</h1><div className="mt-10 space-y-5">{["Five-discipline reviewed scope", "Tutor and timed practice modes", "Clear analytics down to the topic"].map((item) => <div key={item} className="flex items-center gap-3 text-slate-300"><CheckCircle2 className="size-5 text-emerald-400" />{item}</div>)}</div></div>
