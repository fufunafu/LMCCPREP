...
import { PUBLIC_EXAM_LABELS } from "@/lib/public-subjects";
...
export function HeroSection({ checkoutAvailable = false }: { checkoutAvailable?: boolean }) {
  return (
    <section className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 pb-24 pt-16 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:pb-32 lg:pt-24">
      <div><div className="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-3 py-1.5 text-sm font-medium text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300"><GraduationCap className="size-4" />Built for the MCCQE and USMLE</div>
        ...
        <p className="mt-7 max-w-xl text-lg leading-8 text-slate-600 dark:text-slate-300">{checkoutAvailable ? "A focused question bank for medical students and graduates preparing for the MCCQE Part I or the USMLE." : "A focused, invite-only question bank for medical students and graduates preparing for the MCCQE Part I or the USMLE."}</p>
...
export function ExploreSection({ showSubjects, showPricing }: { showSubjects: boolean; showPricing: boolean }) {
  const cards: Array<{ href: string; title: string; copy: string; icon: typeof Sparkles }> = [
    ...
    ...(showPricing ? [{ href: "/pricing", title: "Pricing", copy: "Monthly, three-month, or annual subscription access in CAD.", icon: WalletCards }] : []),
...
export function PricingSection({ plans, standalone = false, checkoutAvailable = false }: { plans: BillingPlan[]; standalone?: boolean; checkoutAvailable?: boolean }) {
  ...
  const savings = (plan: BillingPlan) => {
    if (!monthly || !plan.amountCad || plan.months <= 1) return null;
    const amount = monthly * plan.months - plan.amountCad;
    return amount > 0 ? { amount, percent: Math.round((amount / (monthly * plan.months)) * 100) } : null;
  };
  const columns = plans.length >= 3 ? "max-w-5xl md:grid-cols-3" : "max-w-3xl md:grid-cols-2";
  return (
    <section id="pricing" aria-labelledby="pricing-title" className={`scroll-mt-20 border-b bg-white dark:bg-slate-950/50 ${standalone ? "pb-24 pt-12 sm:pt-16" : "py-24"}`}>
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mx-auto max-w-2xl text-center"><WalletCards className="mx-auto size-8 text-emerald-700" /><p className="mt-5 text-sm font-semibold text-emerald-800 dark:text-emerald-400">Simple subscription access</p><Heading id="pricing-title" className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">One question bank, simple billing options.</Heading><p className="mt-4 leading-7 text-muted-foreground">{checkoutAvailable ? "Choose monthly, three-month, or annual access, manage payment methods and invoices through Stripe, and cancel any time from the customer portal." : "Invited learners can choose monthly, three-month, or annual access, manage payment methods and invoices through Stripe, and cancel from the customer portal."}</p><form action={startDemoSession.bind(null, "/dashboard")} className="mt-5"><Button type="submit" variant="outline">Try the free demo, no card required</Button></form></div>
        <div className={`mx-auto mt-10 grid gap-4 ${columns}`}>{plans.map((plan) => { const saved = savings(plan); return <div key={plan.key} className={`rounded-2xl border bg-background p-6 ${plan.key === "annual" ? "border-emerald-300 dark:border-emerald-800" : ""}`}><div className="flex items-center justify-between gap-3"><h3 className="text-lg font-semibold">{plan.name}</h3>{saved ? <Badge className="bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">Save ${saved.amount} ({saved.percent}%)</Badge> : null}</div><p className="mt-4 text-3xl font-semibold">{plan.formattedPrice ?? "CAD pricing"}</p><p className="mt-1 text-sm text-muted-foreground">{plan.formattedPrice ? plan.cadence : "Shown at Checkout"}</p><ul className="mt-6 space-y-3 text-sm">{["Available reviewed questions and explanations", "Tutor and timed sessions", "Progress, flags, and notes", ...(plan.trialDays ? [`${plan.trialDays}-day trial before paid billing`] : []), "Self-service billing portal"].map((feature) => <li key={feature} className="flex gap-2"><CheckCircle2 className="mt-0.5 size-4 shrink-0 text-emerald-600" />{feature}</li>)}</ul>{checkoutAvailable ? <Link href={`/billing?plan=${plan.key}`} className={buttonVariants({ className: "mt-6 w-full bg-emerald-800 hover:bg-emerald-900" })}>Subscribe</Link> : <Link href="/request-access" className={buttonVariants({ variant: "outline", className: "mt-6 w-full" })}>Request invite</Link>}</div>; })}</div>
      </div>
...
export function FaqSection({ checkoutAvailable = false }: { checkoutAvailable?: boolean }) {
  return <section id="faq" className="mx-auto max-w-4xl scroll-mt-20 px-5 py-24 sm:px-8"><div className="text-center"><p className="text-sm font-semibold text-emerald-800 dark:text-emerald-400">Questions, answered</p><h2 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">{checkoutAvailable ? "Before you subscribe" : "Before you request access"}</h2></div><div className="mt-10"><FaqList items={marketingFaqs(checkoutAvailable)} /></div></section>;
}

/** Full, topic-grouped FAQ for the dedicated page. */
export function ExtendedFaqSection({ checkoutAvailable = false }: { checkoutAvailable?: boolean }) {
  const groups = extendedFaqGroups(checkoutAvailable);
  return <section id="faq" className="mx-auto max-w-4xl scroll-mt-20 px-5 pb-24 pt-8 sm:px-8"><nav aria-label="FAQ topics" className="flex flex-wrap gap-2 text-sm">{groups.map((group) => <a key={group.title} href={`#faq-${group.title.toLowerCase().replaceAll(/[^a-z]+/g, "-")}`} className="rounded-full border bg-background px-3 py-1.5 hover:border-emerald-600">{group.title}</a>)}</nav><div className="mt-12 space-y-14">{groups.map((group) => { const id = `faq-${group.title.toLowerCase().replaceAll(/[^a-z]+/g, "-")}`; return <div key={group.title} id={id} className="scroll-mt-28"><h2 className="text-2xl font-semibold tracking-[-0.03em]">{group.title}</h2><div className="mt-5"><FaqList items={group.items} /></div></div>; })}</div><p className="mt-14 text-center text-sm text-muted-foreground">Still have a question? <Link href="/support" className="font-medium text-emerald-800 underline dark:text-emerald-400">Contact support</Link>.</p></section>;
}

export function AccessSection({ checkoutAvailable = false }: { checkoutAvailable?: boolean }) {
  return <section id="access" className="scroll-mt-20 px-5 py-20 sm:px-8"><div className="relative mx-auto max-w-6xl overflow-hidden rounded-[32px] bg-emerald-800 px-6 py-14 text-white sm:px-12 lg:px-16"><div className="absolute -right-20 -top-24 size-72 rounded-full border-[48px] border-white/10" /><div className="relative grid items-center gap-10 lg:grid-cols-[1fr_.85fr]"><div><ShieldCheck className="size-8 text-emerald-100" /><h2 className="mt-5 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Ready for more focused practice?</h2><p className="mt-4 max-w-xl leading-7 text-emerald-50">{checkoutAvailable ? "Try the no-card demo now, or create an account and subscribe for saved progress and full access." : "Try the no-card demo now, or request an invitation for saved account access."}</p></div>{checkoutAvailable ? <div className="flex flex-col gap-3 sm:flex-row lg:justify-end"><Link href="/signup" className={buttonVariants({ size: "lg", className: "h-12 !bg-white px-6 text-base !text-emerald-900 hover:!bg-emerald-50" })}>Create an account</Link><Link href="/pricing" className={buttonVariants({ variant: "outline", size: "lg", className: "h-12 border-emerald-200 bg-transparent px-6 text-base text-white hover:bg-emerald-700 hover:text-white" })}>See pricing</Link></div> : <AccessRequestForm />}</div></div></section>;
}
