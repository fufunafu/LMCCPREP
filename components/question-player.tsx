...
import { buildExplanationContent, type ExplanationBlock } from "@/lib/explanation-content";
  ...
  return (
    <div className="space-y-3">
      {blocks.map((block, blockIndex) => {
        ...
        if (block.type === "heading") return <p key={key} className="font-semibold">{block.text}</p>;
        if (block.type === "bullets") return <ul key={key} className="list-disc space-y-1 pl-5">{block.items.map((item, itemIndex) => <li key={`${key}-${itemIndex}`}>{item}</li>)}</ul>;
