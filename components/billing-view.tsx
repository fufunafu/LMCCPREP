  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 md:px-8 md:py-8">
      ...
      {summary.mode !== "demo" && canChoosePlan && <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{plans.map((plan) => {
        ...
        return <Card key={plan.key} className={cn(plan.key === "annual" && "border-emerald-300 shadow-sm dark:border-emerald-800")}><CardHeader><CardTitle>{plan.name}</CardTitle><CardDescription>Access to approved questions in Internal Medicine, Pediatrics, PMCH, Psychiatry, and Surgery. Obstetrics and Gynecology is not yet included.</CardDescription></CardHeader><CardContent><div className="mb-6"><p className="text-3xl font-semibold tracking-tight">{plan.formattedPrice ?? "CAD pricing"}</p><p className="mt-1 text-sm text-muted-foreground">{plan.formattedPrice ? plan.cadence : "Shown at Checkout"}</p></div><ul className="mb-6 space-y-3 text-sm">{features.map((feature) => <li key={feature} className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-emerald-600" />{feature}</li>)}</ul><Button className="w-full bg-emerald-800 hover:bg-emerald-900" disabled={!summary.configured || !plan.configured || Boolean(pendingPlan)} onClick={() => startCheckout(plan.key)}><CreditCard />{pendingPlan === plan.key ? "Opening Checkout…" : `Choose ${plan.name.toLowerCase()}`}</Button>{(!summary.configured || !plan.configured) && <p className="mt-3 text-center text-xs text-muted-foreground">This plan becomes available after Stripe test configuration is added.</p>}</CardContent></Card>;
      })}</div>}
