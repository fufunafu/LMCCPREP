  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 md:px-8 md:py-8">
      ...
      {summary.mode !== "demo" && canChoosePlan && <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{plans.map((plan) => {
        ...
        return <Card key={plan.key} className={cn(plan.key === "annual" && "border-emerald-300 shadow-sm dark:border-emerald-800")}><CardHeader><CardTitle>{plan.examId === "usmle" ? "USMLE" : "MCCQE"} · {plan.name}</CardTitle><CardDescription>{plan.examId === "usmle" ? "Access to available USMLE Step 1 and Step 2 CK questions. MCCQE access is sold separately." : "Access to approved questions in Internal Medicine, Pediatrics, PMCH, Psychiatry, and Surgery. Obstetrics and Gynecology is not yet included. USMLE access is sold separately."}</CardDescription></CardHeader><CardContent><div className="mb-6"><p className="text-3xl font-semibold tracking-tight">{plan.formattedPrice ?? "CAD pricing"}</p><p className="mt-1 text-sm text-muted-foreground">{plan.formattedPrice ? plan.cadence : "Shown at Checkout"}</p></div><ul className="mb-6 space-y-3 text-sm">{features.map((feature) => <li key={feature} className="flex gap-2"><Check className="mt-0.5 size-4 shrink-0 text-emerald-600" />{feature}</li>)}</ul><Button className="w-full bg-emerald-800 hover:bg-emerald-900" disabled={!summary.configured || !plan.configured || Boolean(pendingPlan)} onClick={() => startCheckout(plan.key)}><CreditCard />{pendingPlan === plan.key ? "Opening Checkout…" : `Choose ${plan.name.toLowerCase()} · ${plan.examId === "usmle" ? "USMLE" : "MCCQE"}`}</Button>{(!summary.configured || !plan.configured) && <p className="mt-3 text-center text-xs text-muted-foreground">This plan is not available yet.</p>}</CardContent></Card>;
      })}</div>}
      ...
      {summary.hasAccess && !summary.subscriptionId && !summary.granted && summary.mode !== "demo" && <Card className={cn(canChoosePlan && "mt-6")}><CardHeader><CardTitle>Current access</CardTitle><CardDescription>Subscription enforcement is off, so your account keeps access while billing is finalized.</CardDescription></CardHeader><CardContent><Link href="/dashboard" className={buttonVariants()}>Continue to dashboard</Link></CardContent></Card>}
