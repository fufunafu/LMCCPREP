  ...
  return (
    <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 md:px-8 md:py-8">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
        <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle className="text-base">Study activity</CardTitle><p className="mt-1 text-xs text-muted-foreground">Questions attempted over the last 12 weeks</p></div><div aria-hidden="true" className="hidden items-center gap-1.5 text-[10px] text-muted-foreground sm:flex"><span>Less</span>{["bg-muted", "bg-emerald-200", "bg-emerald-400", "bg-emerald-600"].map((color) => <span key={color} className={`size-[11px] rounded-[2px] ${color}`} />)}<span>More</span></div></CardHeader><CardContent><div aria-hidden="true" className="overflow-x-auto pb-1"><div className="grid w-max grid-flow-col grid-rows-7 gap-[3px]">{stats.activity.map((day) => <span key={day.date} title={`${day.date}: ${day.attempted} questions`} className={`size-[11px] rounded-[2px] ${heat(day.attempted)}`} />)}</div></div><table className="sr-only"><caption>Weekly study activity for the last 12 weeks</caption><thead><tr><th>Week</th><th>Attempted</th><th>Correct</th></tr></thead><tbody>{activityWeeks.map((week) => <tr key={week.label}><td>{week.label}</td><td>{week.attempted}</td><td>{week.correct}</td></tr>)}</tbody></table><div className="mt-4 flex items-center justify-between text-xs text-muted-foreground"><span>{recentTotal.toLocaleString()} questions in the last 12 weeks</span><span className="font-medium text-foreground">{bestDay ? `Most active on ${bestDay}` : "No activity yet"}</span></div></CardContent></Card>
