...
import { ArrowRight, BarChart3, Bookmark, CheckCircle2, Clock3, GraduationCap, LineChart, Menu, ShieldCheck, Sparkles, TimerReset, X } from "lucide-react";
...
const faqs = [
  ["Who is LMCC Prep for?", "It is designed for Canadian medical students and graduates preparing for the MCCQE Part I."],
  ["How do I get access?", "LMCC Prep is currently private and invite-only. Send a request and we will follow up when a place is available."],
...
export function MarketingPage({ subjects }: { subjects: Subject[] }) {
    ...
    <div className="min-h-screen overflow-hidden bg-[#f8fbfa] dark:bg-[#07110e]">
      <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-[#f8fbfa]/85 backdrop-blur-xl dark:border-white/10 dark:bg-[#07110e]/85">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8"><Logo className="text-lg" />
          <nav aria-label="Marketing navigation" className="hidden items-center gap-8 text-sm font-medium text-slate-600 dark:text-slate-300 md:flex"><Link href="#features">Features</Link><Link href="#subjects">Subjects</Link><Link href="#faq">FAQ</Link></nav>
        ...
        {menu && <div className="border-t px-5 py-4 md:hidden"><div className="flex flex-col gap-1">{[["Features", "#features"], ["Subjects", "#subjects"], ["FAQ", "#faq"], ["Sign in", "/login"]].map(([label, href]) => <Link key={label} href={href} onClick={() => setMenu(false)} className="rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted">{label}</Link>)}</div></div>}
      </header>
      <main>
