...
const faqs = [
  ["Who is Montreal QBank for?", "It is designed for Canadian medical students and graduates preparing for the MCCQE Part I."],
  ["How do I get access?", "Montreal QBank is currently private and invite-only. Send a request and we will follow up when a place is available."],
  ["What is included?", "The bank covers Pediatrics, Psychiatry, Medicine, Population Medicine and Community Health, and Surgery, with detailed explanations."],
  ["Can I use it on my phone?", "Yes. Every practice surface is designed to work comfortably on iPhone-sized screens and larger."],
];
...
export function MarketingPage({ subjects, plans }: { subjects: Subject[]; plans: BillingPlan[] }) {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f8fbfa] dark:bg-[#07110e]">
      ...
      <main>
        <section className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 pb-24 pt-16 sm:px-8 lg:grid-cols-[1.02fr_.98fr] lg:pb-32 lg:pt-24">
          <div><div className="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-3 py-1.5 text-sm font-medium text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300"><GraduationCap className="size-4" />Built for the MCCQE Part I</div>
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-slate-600 dark:text-slate-300">A focused question bank for Canadian medical students, with {questionTotal} questions across the five subjects that matter most.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="#access" className={buttonVariants({ size: "lg", className: "h-12 bg-emerald-800 px-5 text-base hover:bg-emerald-900" })}>Request invite <ArrowRight /></Link><Link href="/login" className={buttonVariants({ variant: "outline", size: "lg", className: "h-12 px-5 text-base" })}>I have access</Link></div>
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-600 dark:text-slate-400">{["Invite-only", "No distractions", "Made for mobile"].map((item) => <span key={item} className="inline-flex items-center gap-1.5"><CheckCircle2 className="size-4 text-emerald-600" />{item}</span>)}</div>
          </div>
