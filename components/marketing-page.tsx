...
export function MarketingPage({ subjects, plans, billingAvailable }: { subjects: Subject[]; plans: BillingPlan[]; billingAvailable: boolean }) {
  const approvedQuestionCount = subjects.reduce((sum, subject) => sum + subject.questionCount, 0);
  const hasApprovedSubjects = approvedQuestionCount > 0;
    ...
    <div className="min-h-screen overflow-x-clip bg-[#f8fbfa] dark:bg-[#07110e]">
      <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-[#f8fbfa]/85 backdrop-blur-xl dark:border-white/10 dark:bg-[#07110e]/85">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8"><Logo className="text-lg" />
          <nav aria-label="Marketing navigation" className="hidden items-center gap-8 text-sm font-medium text-slate-600 dark:text-slate-300 md:flex"><Link href="#features">Features</Link>{hasApprovedSubjects ? <Link href="#subjects">Subjects</Link> : null}{billingAvailable ? <Link href="#pricing">Pricing</Link> : null}<Link href="#faq">FAQ</Link></nav>
        ...
        {menu && <div className="border-t px-5 py-4 md:hidden"><div className="flex flex-col gap-1">{[["Features", "#features"], ...(hasApprovedSubjects ? [["Subjects", "#subjects"]] : []), ...(billingAvailable ? [["Pricing", "#pricing"]] : []), ["FAQ", "#faq"], ["Sign in", "/login"]].map(([label, href]) => <Link key={label} href={href} onClick={() => setMenu(false)} className="rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted">{label}</Link>)}</div></div>}
      </header>
