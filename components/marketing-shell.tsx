...
export function MarketingShell({ showSubjects, showPricing, children }: { showSubjects: boolean; showPricing: boolean; children: React.ReactNode }) {
    ...
    <div className="min-h-screen overflow-x-clip bg-[#f8fbfa] dark:bg-[#07110e]">
      <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-[#f8fbfa]/85 backdrop-blur-xl dark:border-white/10 dark:bg-[#07110e]/85">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8"><Logo className="text-lg" />
          ...
          <div className="hidden items-center gap-2 md:flex"><ThemeToggle /><Link href="/login" className={buttonVariants({ variant: "ghost", size: "lg" })}>Sign in</Link><Link href="/request-access" className={buttonVariants({ size: "lg", className: "bg-emerald-800 hover:bg-emerald-900" })}>Request access</Link></div>
        ...
        {menu && <div className="border-t px-5 py-4 md:hidden"><div className="flex flex-col gap-1">{[...links, ["Request access", "/request-access"], ["Sign in", "/login"]].map(([label, href]) => <Link key={href} href={href} aria-current={current(href)} onClick={() => setMenu(false)} className="rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted aria-[current=page]:text-emerald-800 dark:aria-[current=page]:text-emerald-400">{label}</Link>)}</div></div>}
      </header>
