...
export function SettingsView({ profile, billing, exams = [] }: { profile?: Profile; billing: BillingSummary; exams?: Exam[] }) {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    startSaving(async () => {
      try {
        const result = await updateProfile({ displayName: String(form.get("name") ?? ""), medicalSchool: String(form.get("school") ?? "") });
