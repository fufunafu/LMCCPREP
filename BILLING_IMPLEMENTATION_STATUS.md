...
| 1. Stripe account and catalog | Test catalog configured | Test product `Montreal QBank` is active with one CA$59 monthly price and one CA$349 annual price, both recurring CAD prices with no trial. The owner approved sharing the existing Stripe account. Persistent test credentials and the webhook endpoint remain pending explicit approval. |
...
- Billing readiness: 24 of 27 preflight checks pass. The three expected failures are the intentionally absent test API key, key-mode validation, and webhook signing secret.
...
- Vercel contains Supabase variables, the Production canonical site URL, approved public prices and support email, completed legal terms, disabled billing safety flags, and both non-secret test price IDs in Preview and Development. Stripe secrets remain absent.
